...
void CalculerEtat(ETAT *Etat, MECANIQUE *Mecanique, CARTE *Carte)
{
	...
	//float DistanceRoueD_Temp = Etat->DistanceRoueD + (Longitudinal.VitesseConsigne + (Angulaire.VitesseConsigne * Mecanique->EntreAxe / 2.0)) * Carte->Te;
	float DistanceRoueG_Temp = QEI2GetDistanceRoue();
	//float DistanceRoueG_Temp = Etat->DistanceRoueG + (Longitudinal.VitesseConsigne - (Angulaire.VitesseConsigne * Mecanique->EntreAxe / 2.0)) * Carte->Te;
...
{
	Param->VitesseConsigne = SeuillerDansInterval(Param->VitesseConsigne, fabs(Param->VitesseDemande));
