...
void ECANIncomingMessage(MESSAGE * MessageRecu)
{
	...
	if ((MessageRecu->SID & 0x0F) == PING) {
		...
		if (MessageRecu->Nombre_Data == 6) {//Données : X_low | X_high | Y_low | Y_high | Angle_low | Angle_high
			...
			//QEIAjouterAngle((float)(MessageRecu->Data[4] + MessageRecu->Data[5]*256) / 1000.0);
			QEI1SetDistanceRoue(QEI1GetDistanceRoue() - Mecanique.EntreAxe * ((float)(MessageRecu->Data[4] + MessageRecu->Data[5]*256) / 1000.0) / 2.0);
			QEI2SetDistanceRoue(QEI2GetDistanceRoue() - Mecanique.EntreAxe * ((float)(MessageRecu->Data[4] + MessageRecu->Data[5]*256) / 1000.0) / 2.0);
			...
			if (MessageRecu->Nombre_Data == 4) { //Données : Mode | Type | Val_low | Val_High
				switch(MessageRecu->Data[1])
				{
					...
					case 'D':
						...
						QEI1Initialize(Mecanique.Nombre_Pas_D, Mecanique.DiametreRoueD); //Pour recalculer le DeltaX
						SendRecu();
						break;
					case 'G':
						Mecanique.DiametreRoueG = (float)(MessageRecu->Data[3]*256 + MessageRecu->Data[2]) / 100000.0;
						QEI2Initialize(Mecanique.Nombre_Pas_G, Mecanique.DiametreRoueD); //Pour recalculer le DeltaX
						SendRecu();
						break;
					default :
						SendIgnorer();
